...
| REQ-9 | Theme colors for external labels | Uses `ExternalColLabelColor` (columns), `ExternalRowLabelColor` (rows), `ConnectorLineColor`/`ConnectorLineThickness` from theme — no new theme properties | 1.3 |
...
- [x] 1.3 Add line pool, MeasureText, external label brushes, ExternalFontSize (REQ-8, REQ-9) `S`
  ...
  - Cached brushes in constructor: `_extLabelBrush` from `theme.ExternalColLabelColor`, `_extRowLabelBrush` from `theme.ExternalRowLabelColor`, `_connectorBrush` from `theme.ConnectorLineColor`

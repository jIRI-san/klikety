...
for PHASE_NUM in $(seq 1 "${PHASE_COUNT}"); do
    ...
    # Simple heuristic: look for "- [ ]" or "- [~]" after the phase heading
...
# Create PR if on GitHub
if [ -z "${ADO_TOKEN:-}" ]; then
    echo "Creating pull request..."
    gh pr create \
        --title "feat: ${PLAN_SLUG}" \
        --body "Autonomous implementation of plan ${PLAN_SLUG}" \
        --head "${BRANCH}" \
        || echo "PR creation failed or already exists."
else
    echo "Creating ADO pull request..."
    az repos pr create \
        --title "feat: ${PLAN_SLUG}" \
        --description "Autonomous implementation of plan ${PLAN_SLUG}" \
        --source-branch "${BRANCH}" \
        --auto-complete false \
        || echo "ADO PR creation failed or already exists."
fi

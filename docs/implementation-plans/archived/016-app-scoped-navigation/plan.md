...
- **Chord key trigger**: app-scope is activated by a chord key (default `OemPeriod` / `.`, configurable under `appScope.chordKey`). Follows the same mode-lock rule as other chord keys — must be pressed before any nav/arrow/action key (`_modeLocked == false`). Once activated, stays app-scoped until overlay closes. Auto-repeat guarded: `if (_appScoped) return;` at the top of the chord handler prevents redundant Hide→Show cycles from OS key repeat.
...
- **Config structure**: new `appScope` section at the top level of config with `chordKey` (default `OemPeriod`). Config migration v5→v6 (after macros v4→v5). v5→v6 unconditional — macros migration (v4→v5) is a prerequisite.
...
| REQ-10 | Config: `appScope` section with `chordKey` (default `OemPeriod`) | Config deserializes correctly; migration v5→v6 adds section if missing | 3.1, 3.2 |

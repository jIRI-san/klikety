...
public sealed class GridRenderer : IGridRenderer {
  ...
  private void RenderExternalColumnLabels(IReadOnlyList<GridCell> cells, Rect region) {
    ...
    for (int c = 0; c < cols && c < cells.Count; c++) {
      ...
      double halfLabelW = tb.DesiredSize.Width / 2;

      // Above
      Canvas.SetLeft(tb, colCenter - halfLabelW);
      Canvas.SetTop(tb, gridTop - labelMargin);
      _canvas.Children.Add(tb);
      _canvas.Children.Add(CreateConnectorV(colCenter, gridTop - labelMargin + tb.DesiredSize.Height + 2, gridTop, connectorBrush));

      // Below
      var tbBottom = CreateExternalLabel(cellLabel.First, extLabelBrush, fontFamily, fontSize, fontWeight);
      tbBottom.Measure(new Size(cellDip.Width, labelMargin));
      Canvas.SetLeft(tbBottom, colCenter - halfLabelW);
      Canvas.SetTop(tbBottom, gridBottom + labelMargin - tbBottom.DesiredSize.Height);
      _canvas.Children.Add(tbBottom);
      _canvas.Children.Add(CreateConnectorV(colCenter, gridBottom, gridBottom + labelMargin - tbBottom.DesiredSize.Height - 2, connectorBrush));
    }
  }
  ...
  private void RenderExternalRowLabels(IReadOnlyList<GridCell> cells, Rect region) {
    ...
    for (int r = 0; r < rows; r++) {
      ...
      double halfLabelH = tb.DesiredSize.Height / 2;

      // Left — centered in margin area
      Canvas.SetLeft(tb, gridLeft - labelMargin / 2 - tb.DesiredSize.Width / 2);
      Canvas.SetTop(tb, rowCenter - halfLabelH);
      _canvas.Children.Add(tb);
      _canvas.Children.Add(CreateConnectorH(gridLeft - labelMargin / 2 + tb.DesiredSize.Width / 2 + 2, rowCenter, gridLeft, connectorBrush));

      // Right — centered in margin area
      var tbRight = CreateExternalLabel(cellLabel.Second, extLabelBrush, fontFamily, fontSize, fontWeight);
      tbRight.Measure(new Size(labelMargin, cellDip.Height));
      Canvas.SetLeft(tbRight, gridRight + labelMargin / 2 - tbRight.DesiredSize.Width / 2);
      Canvas.SetTop(tbRight, rowCenter - halfLabelH);
      _canvas.Children.Add(tbRight);
      _canvas.Children.Add(CreateConnectorH(gridRight, rowCenter, gridRight + labelMargin / 2 - tbRight.DesiredSize.Width / 2 - 2, connectorBrush));
    }
  }
  ...
  private Line CreateConnectorV(double x, double y1, double y2, Brush stroke) =>
      new() {
        X1 = x, Y1 = y1,
        X2 = x, Y2 = y2,
        Stroke = stroke,
  ...
  private Line CreateConnectorH(double x1, double y, double x2, Brush stroke) =>
      new() {
        X1 = x1, Y1 = y,
        X2 = x2, Y2 = y,
        Stroke = stroke,

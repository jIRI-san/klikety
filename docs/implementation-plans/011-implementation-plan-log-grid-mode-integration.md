...
- [ ] 1.2 Add LogGrid key-policy validation: mode-scoped helper computes effective axis keys during bootstrap and exposes cached effective keys/availability for session activation (`Take(10)` if >10, mark only LogGrid unavailable if <10); ensure no path traversal/injection concerns by constraining to in-memory key arrays only; emit one startup warning via existing violations/tray notification pipeline (no duplicate runtime warnings) without blocking other modes. (REQ-9, REQ-13, RISK-3) [after: 1.1] `M`
